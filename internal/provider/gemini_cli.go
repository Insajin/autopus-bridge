...
// executeToolLoopViaCLI는 tool_loop 모드 요청을 Gemini CLI 프롬프트 기반 시뮬레이션으로 처리합니다.
// 도구 정의와 대화 이력을 프롬프트에 인코딩하여 모델이 구조화된 JSON 블록으로 응답하도록 유도합니다.
func (p *GeminiCLIProvider) executeToolLoopViaCLI(ctx context.Context, req ExecuteRequest) (*ExecuteResponse, error) {
